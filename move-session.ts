 ...
 *
 * When invoked with no arguments, spawns zoxide query --interactive (fzf-based picker)
 * so the user can interactively select a directory from their zoxide database.
 *
 ...
 *
 * Usage:
 *   /move-session              - Select target directory interactively via zoxide
 *   /move-session <targetCwd>  - Move to a specific directory
 *   /move-session $main-worktree - Move to the repository's main git worktree
 */
...
/** Restore pi's TUI terminal state after tearing it down for zoxide cancellation. */
function restorePiTui(): void {
    process.stdout.write("\x1b[?25l");    // Hide cursor
    process.stdout.write("\x1b[?2004h");  // Enable bracketed paste
    process.stdout.write("\x1b[<s");      // Push Kitty keyboard protocol
    if (process.stdin.isTTY) {
        process.stdin.setRawMode(true);
    }
    process.stdin.resume();              // Resume 'data' events (TUI input processing)
}

async function pickPathWithZoxide(): Promise<string | null> {
    // Tear down pi's TUI rendering so zoxide/fzf can take over the terminal cleanly.
    // The TUI has an active input listener on process.stdin and a render loop that
    // writes to stdout. We need to stop both to prevent interference with fzf.
    process.stdin.pause();               // Stop 'data' events (TUI input processing)
    ...
    return new Promise((resolve, reject) => {
        const child = spawn("zoxide", ["query", "--interactive"], {
            stdio: ["inherit", "pipe", "inherit"],
            env: { ...process.env, FZF_DEFAULT_OPTS: "--no-height" },
        });
        ...
        child.on("close", (code) => {
            // Always restore pi's TUI after zoxide exits, regardless of success or failure.
            // If we skip restoration on success, any subsequent error (fork, stat, etc.)
            // in the handler will leave the TUI suspended.
            restorePiTui();
            if (code !== 0) {
                resolve(null);
                return;
        ...
        child.on("error", (err: NodeJS.ErrnoException) => {
            // ENOENT means zoxide is not installed — signal cancellation
            restorePiTui();
            if (err.code === "ENOENT") {
                resolve(null);
            } else {
...
export default function (pi: ExtensionAPI) {
    ...
    pi.registerCommand("move-session", {
        description: "Move session to another directory (interactively via zoxide if no argument given) and relaunch pi there",
        ...
        handler: async (args, ctx) => {
            ...
            let rawTargetCwd = args.trim();
            if (!rawTargetCwd) {
                // Spawn zoxide's interactive fzf picker to select a target directory
                try {
                    const picked = await pickPathWithZoxide();
                    if (picked === null) {
                        ctx.ui.notify(
                            "No path selected (zoxide unavailable or cancelled). " +
                            "Usage: /move-session <targetCwd>, or install zoxide for interactive picking.",
                            "error",
                        );
                        return;
                    }
                    ...
                    ctx.ui.notify(
                        `Could not launch zoxide: ${error?.message ?? String(error)}. ` +
                        "Usage: /move-session <targetCwd> or install zoxide for interactive picking.",
                        "error",
                    );
                    return;
